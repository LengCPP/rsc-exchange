...
import { FiArrowDownLeft, FiArrowUpRight, FiCheckCircle, FiClock, FiXCircle } from "react-icons/fi"
...
function LoanCard({ loan, type }: { loan: any, type: 'incoming' | 'outgoing' }) {
  const queryClient = useQueryClient()
  const { showSuccessToast, showErrorToast } = useCustomToast()

  const respondMutation = useMutation({
    mutationFn: (accept: boolean) => LoansService.respondToLoanRequest({ id: loan.id, accept }),
    onSuccess: () => {
    ...
    <Box p={4} borderWidth="1px" borderRadius="lg" bg="bg.panel" boxShadow="sm">
      <Flex justify="space-between" align="start" mb={4}>
        <VStack align="start" gap={0}>
          <Text fontWeight="bold" fontSize="lg">{loan.item.title}</Text>
          <Text fontSize="xs" color="fg.muted">
            {type === 'incoming' ? `Borrower: ${loan.requester.full_name || loan.requester.email}` : `Owner: ${loan.owner.full_name || loan.owner.email}`}
          </Text>
        </VStack>
        <Badge 
          colorPalette={
            ...
            loan.status === 'accepted' ? 'blue' : 
            loan.status === 'rejected' ? 'red' : 'gray'
          }
        >
      ...
      <HStack fontSize="sm" gap={4} mb={4}>
        <VStack align="start" gap={0}>
          <Text fontSize="2xs" color="fg.muted" fontWeight="bold" textTransform="uppercase">From</Text>
          <Text>{format(new Date(loan.start_date), "MMM d, yyyy")}</Text>
        </VStack>
        <VStack align="start" gap={0}>
          <Text fontSize="2xs" color="fg.muted" fontWeight="bold" textTransform="uppercase">To</Text>
      ...
      <Flex gap={2} mt="auto">
        {type === 'incoming' && isPending && (
          <>
            <Button size="xs" colorPalette="green" onClick={() => respondMutation.mutate(true)} loading={respondMutation.isPending}>
              <FiCheckCircle /> Accept
            </Button>
            <Button size="xs" colorPalette="red" variant="outline" onClick={() => respondMutation.mutate(false)} loading={respondMutation.isPending}>
              <FiXCircle /> Reject
            </Button>
          </>
        )}
        ...
        {type === 'outgoing' && isAccepted && (
          <Button size="xs" colorPalette="blue" width="full" onClick={() => ratifyMutation.mutate()} loading={ratifyMutation.isPending}>
            <FiCheckCircle /> Confirm I have the item
          </Button>
        )}

        {type === 'incoming' && isActive && (
          <Button size="xs" colorPalette="gray" width="full" onClick={() => returnMutation.mutate()} loading={returnMutation.isPending}>
            Mark as Returned
          </Button>
        )}
      </Flex>
...
function LoansPage() {
  ...
  const borrowedItems = outgoing?.data.filter(l => l.status === 'active' || l.status === 'return_pending') || []
  const pendingOutgoing = outgoing?.data.filter(l => l.status === 'pending' || l.status === 'accepted') || []
  const pendingIncoming = incoming?.data.filter(l => l.status === 'pending' || l.status === 'return_pending') || []

  return (
    <Container maxW="full" pb={20}>
      <Heading size="lg" py={12}>Loans & Possession</Heading>
        ...
        <Box>
          ...
          {borrowedItems.length === 0 ? (
            <Box p={8} textAlign="center" borderWidth="1px" borderRadius="lg" borderStyle="dashed">
              <Text color="fg.muted">You are not currently borrowing any items.</Text>
            </Box>
          ) : (
              ...
              {borrowedItems.map(loan => (
                <VStack key={loan.id} align="stretch" gap={2}>
                  ...
                  {loan.status === 'return_pending' && (
                    <Badge colorPalette="blue" variant="subtle" width="full" justifyContent="center">
                      Waiting for owner
                    </Badge>
                  )}
                </VStack>
        ...
        <SimpleGrid columns={{ base: 1, lg: 2 }} gap={12}>
          ...
          <Box>
            ...
            {pendingIncoming.length === 0 ? (
              <Text fontSize="sm" color="fg.muted">No pending requests or returns for your items.</Text>
            ) : (
          ...
          <Box>
            ...
            {pendingOutgoing.length === 0 ? (
              <Text fontSize="sm" color="fg.muted">You have no pending requests.</Text>
            ) : (
              <VStack align="stretch" gap={4}>
